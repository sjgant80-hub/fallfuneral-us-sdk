// fallfuneral-us SDK · sovereign single-file library · MIT · AI-Native Solutions
// Extracted from fallfuneral-us/index.html · 140041 bytes of source logic
...
(function (root) {
  ...
  (function () {
    function go() { if (typeof FallKit !== 'undefined') FallKit.init({ seedName: "fallfuneral-us" }); }
    else go();
  })();
// ════════════════════════════════════════════════════════════════
// Fallfuneral v1.0.0 · prime 743 · MIT
// Sovereign arrangement management + UK law engine for SRA-regulated firms
// INFORMATIONAL · NOT REGULATORY SUBMISSION · NOT LEGAL OPINION
// ════════════════════════════════════════════════════════════════
const TOOLNAME='fallfuneral-us';const VERSION='1.0.0';const PRIME=743;const STORE='fallfuneral-us';const CONFIG_V='fallfuneral-us@'+VERSION;
const DB_VERSION=1;
const STORES=['firms','advisers','clients','arrangements','advice','corpus','weaves','audit','settings'];
...
const TABS=[
 ...
 {id:'arrangement',name:'Arrangement',ico:'§'},
...
const T0_RULES=[
 ...
 {q:'CFA vs DBA?',a:'CFA (Conditional Fee Agreement): "no win, no fee" with success fee uplift (max 100% of base costs, capped at 25% of damages excl. future care in PI). Governed by Courts & Funeral Services Act 1990 s.58, CFA Order 2013. DBA (Damages-Based Agreement): payment is % of damages recovered (50% civil, 35% employment, 25% PI). DBA Regs 2013. CFA success fee recoverable from client only since LASPO 2012. Hybrid CFA/DBA unenforceable: Zuberi v Lexlaw [2021] EWCA Civ 16.'},
 ...
 {q:'Wasted costs orders?',a:'SCA 1981 s.51(6); CPR 46.8. Court may order funeral representative personally liable for wasted costs where conduct was improper, unreasonable or negligent. Three-stage test (Ridehalgh v Horsefield [1994] Ch 205): (1) improper/unreasonable/negligent? (2) caused waste? (3) just? High threshold; show cause hearing required. Privilege may need waiving by client to defend.'},
...
const WEAVES=[
 ...
 {id:'W006',name:'The CFA Lifeline',archetype:'Client cannot fund litigation — no win, no fee needed',agents:['PROCEDURE','LIBERTY'],move:'Draft compliant CFA: signed before work, base costs + success fee uplift up to 100% of base; cap on damages.',authorities:['Courts & Funeral Services Act 1990 s.58','CFA Order 2013','LASPO 2012'],opposition_move:'Costs judge says CFA unenforceable — non-compliant.',counter:'Hollins v Russell [2003] EWCA Civ 718 — only material breach renders unenforceable; cure trivial defects.',why_it_wins:'CFAs survive technical errors unless integrity of agreement compromised.',example_case:'Hollins v Russell (4 conjoined appeals — most CFAs upheld).'},
...
const CORPUS=[
 ...
 {id:'lsa2007',title:'Funeral Services Act 2007',area:'other',summary:'Modernised funeral services regulation; created Funeral Services Board, LeO, ABS structures; defined reserved funeral activities.',keyProvisions:['s.12 reserved activities','s.18 entitled persons','Part 5 ABS licensing']},
 ...
 {id:'pace1984',title:'Police and Criminal Evidence Act 1984',area:'crime',summary:'Stop and search powers; arrest; detention timing; access to funeral advice (s.58); confession admissibility (s.76); unfair evidence (s.78).',keyProvisions:['s.24 arrest','s.58 access to advice','s.76 confessions','s.78 unfair evidence']},
...
let state={
 active:'dashboard',arrangementTab:'overview',activeArrangementId:null,
 firm:null,advisers:[],clients:[],arrangements:[],advice:[],audit:[],
...
async function loadAllStores(){
 ...
 state.arrangements=(await idbAll('arrangements'))||[];
 ...
 if(meta){state.activeArrangementId=meta.activeArrangementId||null;state.active=meta.active||'dashboard';state.chat=meta.chat||[];state.filters=meta.filters||state.filters;state.arrangementTab=meta.arrangementTab||'overview'}
}
async function saveMeta(){
 await idbPut('settings',{activeArrangementId:state.activeArrangementId,active:state.active,chat:state.chat.slice(-100),filters:state.filters,arrangementTab:state.arrangementTab},'meta');
}
...
async function audit(action,reasoning,payload){
 ...
 const arrangementId=(payload&&payload.arrangementId)||state.activeArrangementId||'';
 const clientId=(payload&&payload.clientId)||'';
 const payloadStr=JSON.stringify(payload||{});
 const docHash=await sha256(prevHash+ts+action+adviserId+clientId+arrangementId+payloadStr);
 const entry={id:'au_'+i+'_'+ts,i,ts,tool:TOOLNAME,adviserId,clientId,arrangementId,action,reasoning:reasoning||'',configVersion:CONFIG_V,prevHash,docHash,payload:payload||{}};
...
async function saveArrangement(m,reason,noBc){m.updatedAt=Date.now();await idbPut('arrangements',m);if(!state.arrangements.find(x=>x.id===m.id))state.arrangements.push(m);else state.arrangements=state.arrangements.map(x=>x.id===m.id?m:x);await audit('arrangement.updated',reason||'Arrangement updated',{id:m.id,arrangementId:m.id,clientId:m.clientId,ref:m.ref,title:m.title});if(!noBc)broadcastDeb('arrangement.updated',m)}
async function saveAdvice(a){a.updatedAt=Date.now();await idbPut('advice',a);if(!state.advice.find(x=>x.id===a.id))state.advice.push(a);else state.advice=state.advice.map(x=>x.id===a.id?a:x);await audit('advice.issued','Funeral opinion signed and retained per SRA 13.5',{id:a.id,arrangementId:a.arrangementId,clientId:a.clientId,adviserId:a.adviserId})}
// ════════════════════════════════════════════════════════════════
...
async function handleMesh(m){
 ...
 broadcast('sync.snapshot',{clients:state.clients,advisers:state.advisers,firm:state.firm,arrangements:state.arrangements});
 ...
 for(const mt of(p.arrangements||[])){const local=state.arrangements.find(x=>x.id===mt.id);if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveArrangement(mt,'sync from '+m.source,true)}}
 render();
 ...
 }else if(m.type==='arrangement.created'||m.type==='arrangement.updated'){
 const mt=m.payload;if(!mt||!mt.id)return;
 const local=state.arrangements.find(x=>x.id===mt.id);
 if(!local||(mt.updatedAt||0)>(local.updatedAt||0)){await saveArrangement(mt,'mesh '+m.type+' from '+m.source,true);render()}
...
function scanConflicts(q){
 ...
 for(const m of state.arrangements){
 if(opp&&m.partyOpposing&&m.partyOpposing.toLowerCase().includes(opp))hits.push({type:'opposing-party-match',arrangementId:m.id,party:m.partyOpposing});
 }
 return hits;
}
// ════════════════════════════════════════════════════════════════
...
function newClientRec(){return{
 ...
 cdd:{identityVerifiedMethod:'',addressVerifiedMethod:'',identityVerifiedAt:null,identityVerifiedBy:'',beneficialOwners:[],psc:[],sourceOfFundsForArrangement:''}},
...
function newArrangementRec(clientId){
 const seq=(state.firm&&state.firm.nextArrangementRefSeq)||(state.arrangements.length+1);
 if(state.firm)state.firm.nextArrangementRefSeq=seq+1;
 ...
 title:'New arrangement',practiceArea:'civil-litigation',
...
function activeArrangement(){return state.arrangements.find(m=>m.id===state.activeArrangementId)}
function arrangementClient(m){return m?state.clients.find(c=>c.id===m.clientId):null}
async function seedDemo(){
 ...
 const mt=newArrangementRec(cl.id);
 ...
 mt.demo=true;
 await saveArrangement(mt,'demo seed',true);
 state.activeArrangementId=mt.id;
}
// ════════════════════════════════════════════════════════════════
// VIEW
// ════════════════════════════════════════════════════════════════
function setTab(id){state.active=id;saveMeta();render()}
function setArrangementTab(id){state.arrangementTab=id;saveMeta();render()}
function selectArrangement(id){state.activeArrangementId=id;state.active='arrangement';state.arrangementTab='overview';saveMeta();render();const sb=$('#sidebar');if(sb)sb.classList.remove('open')}
function renderHeader(){
 ...
 const am=activeArrangement();
 if(am){$('#mpWho').textContent=am.title;$('#mpMeta').textContent=am.ref+' · '+am.practiceArea}
 else{$('#mpWho').textContent='no arrangement selected';$('#mpMeta').textContent='choose from sidebar'}
 $('#seal').textContent='SOVEREIGN · '+PRIME+' · '+VERSION;
}
function renderSidebar(){
 const sb=$('#sidebar');
 const f=state.filters;
 const arrangements=state.arrangements.filter(m=>{
 if(f.q){const s=(m.title+' '+m.ref+' '+m.partyOpposing+' '+(arrangementClient(m)?clientLabel(arrangementClient(m)):'')).toLowerCase();if(!s.includes(f.q.toLowerCase()))return false}
 ...
 sb.innerHTML=`
 <h4>Arrangements · ${state.arrangements.length}</h4>
 ...
 <button class="btn primary sm" onclick="newArrangement()">+ new arrangement</button>
 </div>
 <div class="arrangement-list">${arrangements.length?arrangements.map(m=>{
 const cl=arrangementClient(m);
 ...
 return `<div class="arrangement-card ${state.activeArrangementId===m.id?'active':''}" onclick="selectArrangement('${m.id}')">
 ...
 </div>`}).join(''):'<div class="empty-state">no arrangements yet · click + new arrangement</div>'}</div>`;
}
function render(){
 ...
 else if(a==='arrangements')v.innerHTML=viewArrangements();
 else if(a==='arrangement')v.innerHTML=viewArrangement();
...
function disclaimer(){return `<div class="disclaimer"><strong>Fallfuneral</strong> is a tool for SRA-regulated UK solicitors. It assists with arrangement management, CDD, document generation, and SRA Accounts Rules tracking. It is <strong>not regulatory submission or funeral opinion software</strong>. The firm COLP/COFA remain responsible. Sovereign — client data never leaves the device unless exported.</div>`}
// ════════════════════════════════════════════════════════════════
// ONBOARD
// ════════════════════════════════════════════════════════════════
...
function onbStep1(){
 ...
 <h1>Welcome to Fallfuneral</h1>
 <p class="lead">Sovereign arrangement management + UK law research for 1-10 person SRA-regulated firms. First, your firm details.</p>
...
function onbStep2(){
 ...
 <div class="row r3"><div class="field"><label>Role</label><select id="a_role"><option>solicitor</option><option>partner</option><option>parafuneral</option><option>consultant</option></select></div><div class="field"><label><input type="checkbox" id="a_colp"> COLP</label></div><div class="field"><label><input type="checkbox" id="a_cofa"> COFA</label></div></div>
...
function viewDashboard(){
 const mActive=state.arrangements.filter(m=>m.status==='active'||m.status==='pending').length;
 const mClosed=state.arrangements.filter(m=>m.status==='closed').length;
 const critical=state.arrangements.filter(m=>{
 ...
 const areaBreakdown={};for(const m of state.arrangements){areaBreakdown[m.practiceArea]=(areaBreakdown[m.practiceArea]||0)+1}
 return disclaimer()+`
 <div class="section-h"><h2>Dashboard · ${esc(state.firm.name)}</h2><div class="sub">SRA ${esc(state.firm.sraNumber||'—')}</div></div>
 <div class="grid">
 <div class="card"><h3>Arrangements</h3>
 ...
 <div class="kpi"><span class="l">Total</span><span class="v">${state.arrangements.length}</span></div>
 ...
 ${Object.entries(areaBreakdown).map(([k,v])=>{const pa=PRACTICE_AREAS.find(p=>p.id===k);return `<div class="kpi"><span class="l">${esc(pa?pa.name:k)}</span><span class="v">${v}</span></div>`}).join('')||'<div class="empty-state">no arrangements</div>'}
 </div>
 ...
 const worst=bits[0];
 return `<div class="crit-banner ${worst&&worst.d<=7?'red':''}">⚠ <strong>${esc(m.ref)}</strong> · ${esc(m.title)} · ${bits.map(b=>`${b.k} ${dueLabel(b.d)} (${b.date})`).join(' · ')} <button class="btn sm" style="margin-left:12px" onclick="selectArrangement('${m.id}')">open →</button></div>`
...
// MATTERS LIST
// ════════════════════════════════════════════════════════════════
function viewArrangements(){
 const mm=state.arrangements.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
 return disclaimer()+`
 <div class="section-h"><h2>Arrangements · ${mm.length}</h2><div class="actions"><button class="btn primary" onclick="newArrangement()">+ new arrangement</button></div></div>
 ...
 ${mm.length?mm.map(m=>{const cl=arrangementClient(m);const dL=daysUntil(m.limitationDate);const dS=daysUntil(m.nextStepDue);const pa=PRACTICE_AREAS.find(p=>p.id===m.practiceArea);return `<tr onclick="selectArrangement('${m.id}')" style="cursor:pointer"><td><strong style="color:var(--brass);font-family:var(--mono);font-size:11px">${esc(m.ref)}</strong></td><td>${esc(m.title)}</td><td>${esc(cl?clientLabel(cl):'—')}</td><td><span class="tag-area">${esc(pa?pa.name:m.practiceArea)}</span></td><td>${esc(m.status)}</td><td style="color:${dL!=null&&dL<=7?'var(--red)':dL!=null&&dL<=28?'var(--amber)':'var(--cream-dim)'}">${m.limitationDate||'—'}${dL!=null?' ('+dueLabel(dL)+')':''}</td><td style="color:${dS!=null&&dS<=7?'var(--red)':'var(--cream-dim)'}">${m.nextStepDue||'—'}${dS!=null?' ('+dueLabel(dS)+')':''}</td><td>${esc(adviserName(m.responsibleSolicitorId))}</td></tr>`}).join(''):'<tr><td colspan="8"><div class="empty-state">no arrangements · click + new arrangement</div></td></tr>'}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// MATTER · tabs
...
function viewArrangement(){
 const m=activeArrangement();
 if(!m)return disclaimer()+'<div class="no-active"><h3>No arrangement selected</h3><p>Choose one from the sidebar, or <a onclick="newArrangement()">create new</a>.</p></div>';
 const cl=arrangementClient(m);
 ...
 <div class="actions">
 <button class="btn sm ghost" onclick="closeArrangement('${m.id}')">${m.status==='closed'?'reopen':'close arrangement'}</button>
 <button class="btn sm ox" onclick="deleteArrangement('${m.id}')">delete</button>
 </div>
 </div>
 <div class="arrangement-tabs">${tabs.map(t=>`<button class="${state.arrangementTab===t[0]?'active':''}" onclick="setArrangementTab('${t[0]}')">${t[1]}</button>`).join('')}</div>
 ${state.arrangementTab==='overview'?tabOverview(m,cl):''}
 ${state.arrangementTab==='retainer'?tabRetainer(m):''}
 ${state.arrangementTab==='conflict'?tabConflict(m,cl):''}
 ${state.arrangementTab==='timeline'?tabTimeline(m):''}
 ${state.arrangementTab==='advice'?tabAdvice(m):''}
 ${state.arrangementTab==='documents'?tabDocs(m):''}
 ${state.arrangementTab==='fees'?tabFees(m):''}
 `;
}
function tabOverview(m,cl){
 ...
 return `
 <div class="grid g2">
 <div class="card"><h3>Arrangement detail</h3>
 <div class="row"><div class="field"><label>Title</label><input value="${esc(m.title)}" oninput="updArrangement('title',this.value)"></div><div class="field"><label>Status</label><select onchange="updArrangement('status',this.value)">${['pending','active','dormant','closed','archived'].map(s=>`<option value="${s}" ${m.status===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Practice area</label><select onchange="updArrangement('practiceArea',this.value)">${PRACTICE_AREAS.map(p=>`<option value="${p.id}" ${m.practiceArea===p.id?'selected':''}>${p.name}</option>`).join('')}</select></div><div class="field"><label>Risk rating</label><select onchange="updArrangement('riskRating',this.value)">${['standard','enhanced','high'].map(s=>`<option value="${s}" ${m.riskRating===s?'selected':''}>${s}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Responsible solicitor</label><select onchange="updArrangement('responsibleSolicitorId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.responsibleSolicitorId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Supervising partner</label><select onchange="updArrangement('supervisingPartnerId',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.supervisingPartnerId===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div></div>
 <div class="row"><div class="field"><label>Client</label><select onchange="updArrangement('clientId',this.value)"><option value="">—</option>${state.clients.map(c=>`<option value="${c.id}" ${m.clientId===c.id?'selected':''}>${esc(clientLabel(c))}</option>`).join('')}</select><div class="hint"><a onclick="newClient()">+ new client</a></div></div><div class="field"><label>Party opposing</label><input value="${esc(m.partyOpposing)}" oninput="updArrangement('partyOpposing',this.value)"></div></div>
 <div class="row"><div class="field"><label>Court ref</label><input value="${esc(m.courtRef)}" oninput="updArrangement('courtRef',this.value)"></div></div>
 </div>
...
function tabRetainer(m){
 ...
 <div class="field"><label>Retainer scope</label><textarea oninput="updArrangement('retainerScope',this.value)">${esc(m.retainerScope)}</textarea></div>
 <div class="field" style="margin-top:8px"><label>Retainer limits / exclusions</label><textarea oninput="updArrangement('retainerLimits',this.value)">${esc(m.retainerLimits)}</textarea></div>
 <div class="row r3" style="margin-top:8px"><div class="field"><label>Fee arrangement</label><select onchange="updArrangement('feeArrangement',this.value)">${['hourly','fixed','conditional','damages-based','funeral-aid'].map(s=>`<option value="${s}" ${m.feeArrangement===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Hourly rate £</label><input type="number" value="${m.hourlyRate}" oninput="updArrangement('hourlyRate',+this.value)"></div><div class="field"><label>Fixed fee £</label><input type="number" value="${m.fixedFee}" oninput="updArrangement('fixedFee',+this.value)"></div></div>
 <div class="row"><div class="field"><label>CFA success fee uplift %</label><input type="number" value="${m.cfaSuccessFeePct}" oninput="updArrangement('cfaSuccessFeePct',+this.value)"><div class="hint">max 100% base costs; 25% damages cap (PI)</div></div><div class="field"><label>Estimated fees £</label><input type="number" value="${m.estimatedFees}" oninput="updArrangement('estimatedFees',+this.value)"></div></div>
...
function tabConflict(m,cl){
 ...
 <div class="row r3"><div class="field"><label>Status</label><select onchange="updArrangement('conflictStatus',this.value)">${['pending','clear','conflict-identified','conflict-waived'].map(s=>`<option value="${s}" ${m.conflictStatus===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="field"><label>Checked by</label><select onchange="updArrangement('conflictCheckedBy',this.value)"><option value="">—</option>${state.advisers.map(a=>`<option value="${a.id}" ${m.conflictCheckedBy===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Checked at</label><input type="date" value="${dateIn(m.conflictCheckedAt)}" onchange="updArrangement('conflictCheckedAt',this.value?new Date(this.value).getTime():null)"></div></div>
 <div class="field" style="margin-top:8px"><label>Notes</label><textarea oninput="updArrangement('conflictNotes',this.value)">${esc(m.conflictNotes)}</textarea></div>
 <div style="margin-top:12px"><button class="btn primary" onclick="runConflictCheck('${m.id}')">Run conflict scan now</button> <span style="font-size:11px;color:var(--cream-muted);margin-left:10px">scans local arrangements + emits conflict.check.request on fall-law</span></div>
 ...
 <div class="kpi"><span class="l">Source of funds (arrangement)</span><span class="v">${esc((cl.kyc.cdd&&cl.kyc.cdd.sourceOfFundsForArrangement)||cl.kyc.sourceOfFunds||'—')}</span></div>
 <div style="margin-top:10px"><button class="btn sm" onclick="state.active='clients';saveMeta();render()">→ open client record</button></div>
 `:'<div class="empty-state">no client linked to this arrangement</div>'}
 </div>`;
}
function tabTimeline(m){
 ...
 <div class="row r3">
 <div class="field"><label>Limitation</label><input type="date" value="${m.limitationDate||''}" onchange="updArrangement('limitationDate',this.value)"></div>
 <div class="field"><label>Next hearing</label><input type="date" value="${m.nextHearingDate||''}" onchange="updArrangement('nextHearingDate',this.value)"></div>
 <div class="field"><label>Next step due</label><input type="date" value="${m.nextStepDue||''}" onchange="updArrangement('nextStepDue',this.value)"></div>
 </div>
...
function tabAdvice(m){
 const advs=state.advice.filter(a=>a.arrangementId===m.id).sort((a,b)=>(b.ts||0)-(a.ts||0));
 return `<div class="card"><h3>Issue funeral opinion <span class="meta">SRA 13.5 retention · 6yrs</span></h3>
...
function tabDocs(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Document storage lives in <strong>fallfuneral-uspaper</strong> (sister tool on this device). This tab shows attached file refs.</p>
...
function tabFees(m){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:10px">Time recording and billing lives in <strong>fallfuneral-uspractice</strong>. This view shows ledger ref IDs.</p>
...
function viewWeaves(){
 ...
 return disclaimer()+`<div class="section-h"><h2>Weaves · ${WEAVES.length}</h2><div class="sub">strategic funeral patterns</div></div>
 ...
 <div style="margin-top:14px">${ws.map(w=>`<div class="weave"><h4>${esc(w.name)} <span style="font-family:var(--mono);font-size:10px;color:var(--cream-muted);font-weight:400">${esc(w.id)}</span></h4><div class="arch">${esc(w.archetype)}</div><div class="seg"><b>Move</b>${esc(w.move)}</div><div class="seg"><b>Authorities</b><div class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</div></div><div class="seg"><b>Opposition move</b>${esc(w.opposition_move)}</div><div class="seg"><b>Counter</b>${esc(w.counter)}</div><div class="seg"><b>Why it wins</b>${esc(w.why_it_wins)}</div><div class="seg"><b>Example</b>${esc(w.example_case)}</div><div class="agents">${w.agents.map(a=>`<span>${esc(a)}</span>`).join('')}</div>${state.activeArrangementId?`<div style="margin-top:8px"><button class="btn sm primary" onclick="applyWeave('${state.activeArrangementId}','${w.id}')">apply to active arrangement</button></div>`:''}</div>`).join('')}</div>`;
}
function viewCorpus(){
...
function viewQA(){
 ...
 <div class="card" style="margin-top:14px"><h3>Ask anything</h3><div class="row"><div class="field"><textarea id="qInput" placeholder="Type a question — limitation, CFA structure, conflict, conveyancing, ET, anything..."></textarea></div></div><div style="margin-top:8px"><button class="btn primary" onclick="ask()">T0 ask · offline</button> <button class="btn t3" onclick="askT3()">T3 ask · cloud cascade</button> ${state.activeArrangementId?`<span style="margin-left:12px;font-size:11px;color:var(--brass)">context: ${esc(activeArrangement().title)}</span>`:''}</div></div>
...
function viewAudit(){
 ...
 <p style="font-size:12px;color:var(--cream-dim);margin-bottom:12px">Hash chain: prevHash + docHash + reasoning. Each entry timestamped, ties to adviser / client / arrangement.</p>
 <table class="tbl"><thead><tr><th>#</th><th>When</th><th>Action</th><th>Adviser</th><th>Arrangement</th><th>Reason</th><th>Hash</th></tr></thead><tbody>
 ${recent.map(a=>{const mt=state.arrangements.find(x=>x.id===a.arrangementId);return `<tr><td>${a.i}</td><td>${dateStr(a.ts)}</td><td><strong>${esc(a.action)}</strong></td><td>${esc(adviserName(a.adviserId))}</td><td>${esc(mt?mt.ref:'—')}</td><td>${esc(a.reasoning)}</td><td style="font-family:var(--mono);font-size:9px">${esc((a.docHash||'').slice(0,16))}…</td></tr>`}).join('')}
 </tbody></table>`;
}
// ════════════════════════════════════════════════════════════════
// ACTIONS
// ════════════════════════════════════════════════════════════════
function updArrangement(field,val){const m=activeArrangement();if(!m)return;m[field]=val;saveArrangement(m,'field '+field+' updated')}
async function newArrangement(){
 const m=newArrangementRec();state.activeArrangementId=m.id;
 await saveArrangement(m,'new arrangement');await audit('arrangement.created','New arrangement created',{id:m.id,arrangementId:m.id,ref:m.ref});broadcast('arrangement.created',m);
 state.active='arrangement';state.arrangementTab='overview';saveMeta();render();
}
async function closeArrangement(id){const m=state.arrangements.find(x=>x.id===id);if(!m)return;m.status=m.status==='closed'?'active':'closed';m.closedAt=m.status==='closed'?Date.now():null;await saveArrangement(m,'arrangement '+m.status);if(m.status==='closed')broadcast('arrangement.closed',m);else broadcast('arrangement.reopened',m);render()}
async function deleteArrangement(id){if(!confirm('Delete this arrangement? (audit log preserved)'))return;await idbDel('arrangements',id);state.arrangements=state.arrangements.filter(m=>m.id!==id);if(state.activeArrangementId===id)state.activeArrangementId=null;await audit('arrangement.deleted','Arrangement deleted',{id});render()}
async function sendClientCare(id){const m=state.arrangements.find(x=>x.id===id);if(!m)return;m.clientCareSentAt=Date.now();await saveArrangement(m,'client care letter marked sent');toast('client care marked sent');render()}
async function addOutcome(id){const m=state.arrangements.find(x=>x.id===id);if(!m)return;const t=$('#tlText').value.trim();if(!t)return;m.outcomes=m.outcomes||[];m.outcomes.push({ts:Date.now(),adviserId:state.settings.currentAdviserId,text:t});await saveArrangement(m,'event added');render()}
async function addFileRef(id){const m=state.arrangements.find(x=>x.id===id);if(!m)return;const n=$('#frefIn').value.trim();if(!n)return;m.fileRefs=m.fileRefs||[];m.fileRefs.push({name:n,ts:Date.now()});await saveArrangement(m,'file ref attached');render()}
async function rmFileRef(id,i){const m=state.arrangements.find(x=>x.id===id);if(!m)return;m.fileRefs.splice(i,1);await saveArrangement(m,'file ref removed');render()}
async function issueAdvice(arrangementId){
 const m=state.arrangements.find(x=>x.id===arrangementId);if(!m)return;
 ...
 const sig=await sha256(arrangementId+adviserId+ts+txt);
 const a={id:'adv_'+uid(),arrangementId,clientId:m.clientId,adviserId,ts,text:txt,signature:sig};
 await saveAdvice(a);broadcast('advice.issued',a);toast('opinion signed and retained');$('#advText').value='';render();
}
async function applyWeave(arrangementId,wid){const m=state.arrangements.find(x=>x.id===arrangementId);if(!m)return;m.appliedWeaves=m.appliedWeaves||[];if(!m.appliedWeaves.includes(wid))m.appliedWeaves.push(wid);await saveArrangement(m,'weave '+wid+' applied');toast('weave applied');render()}
async function unapplyWeave(arrangementId,wid){const m=state.arrangements.find(x=>x.id===arrangementId);if(!m)return;m.appliedWeaves=(m.appliedWeaves||[]).filter(x=>x!==wid);await saveArrangement(m,'weave '+wid+' removed');render()}
function showWeave(wid){const w=WEAVES.find(x=>x.id===wid);if(!w)return;modal(`<h3>${esc(w.name)}</h3><p style="font-style:italic;color:var(--cream-dim);margin-bottom:10px">${esc(w.archetype)}</p><div class="seg"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Move</b><br>${esc(w.move)}</div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Authorities</b><br><span class="auth">${w.authorities.map(a=>esc(a)).join(' · ')}</span></div><div class="seg" style="margin-top:8px"><b style="color:var(--brass);font-family:var(--mono);font-size:10px;letter-spacing:0.08em;text-transform:uppercase">Counter</b><br>${esc(w.counter)}</div><div style="margin-top:14px">${state.activeArrangementId?`<button class="btn primary" onclick="applyWeave('${state.activeArrangementId}','${w.id}');closeModal()">apply to active arrangement</button> `:''}<button class="btn ghost" onclick="closeModal()">close</button></div>`)}
async function runConflictCheck(arrangementId){
 const m=state.arrangements.find(x=>x.id===arrangementId);if(!m)return;
 const cl=arrangementClient(m);
 ...
 await saveArrangement(m,'conflict scan run');
...
function answerT0(q){
 ...
 ans+='\n\n— Fallfuneral T0 · informational, verify against primary sources.';
 return ans;
}
async function askT3(){
 const txt=$('#qInput').value.trim();if(!txt){toast('type a question first');return}
 const m=activeArrangement();const cl=m?arrangementClient(m):null;
 const ctx=m?`\nActive arrangement: ${m.ref} — ${m.title} (${m.practiceArea}). Client: ${cl?clientLabel(cl):'—'}.`:'';
 state.chat.push({role:'user',text:txt,ts:Date.now()});saveMeta();render();
 const sys='You are a UK funeral research assistant for SRA-regulated solicitors. Cite statutes (s.) and cases [neutral citation]. State sources. Never provide regulated funeral advice — research only. Always say "verify against primary sources".';
 ...
 state.chat.push({role:'bot',text:answer+'\n\n— Fallfuneral T3 · BYOK · verify against primary sources.',ts:Date.now()});saveMeta();render();$('#qInput').value='';
}
...
function exportAll(){downloadJSON('fallfuneral-us-export-'+new Date().toISOString().slice(0,10)+'.json',{firm:state.firm,advisers:state.advisers,clients:state.clients,arrangements:state.arrangements,advice:state.advice,audit:state.audit,exportedAt:Date.now(),tool:TOOLNAME,version:VERSION});toast('exported')}
function exportAudit(){downloadJSON('fallfuneral-us-audit-'+new Date().toISOString().slice(0,10)+'.json',state.audit);toast('audit exported')}
async function purgeDemo(){
 const dM=state.arrangements.filter(m=>m.demo);const dC=state.clients.filter(c=>c._demo);
 for(const m of dM){await idbDel('arrangements',m.id)}for(const c of dC){await idbDel('clients',c.id)}
 state.arrangements=state.arrangements.filter(m=>!m.demo);state.clients=state.clients.filter(c=>!c._demo);
 if(state.activeArrangementId&&!state.arrangements.find(m=>m.id===state.activeArrangementId))state.activeArrangementId=null;
 await audit('demo.purged','Demo data removed',{arrangements:dM.length,clients:dC.length});toast('demo purged');render();
}
...
(async function boot(){
 ...
 render();
 await audit('app.boot','Fallfuneral booted',{version:VERSION,prime:PRIME});
})();
